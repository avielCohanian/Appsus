import { storageService } from '../../../service/service.async';
...
function addReview(review, bookId) {
  return getById(bookId).then((book) => {
    ...
    return storageService.put(BOOKS_KEY, book).then((updatedBook) => {
      return updatedBook;
    });
  });
}
...
function searchBooks(name) {
  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${name}`).then((res) => {
    return res.data;
  });
}
